...
@SuppressWarnings("all") // suppress warnings in this file to remove clutter.
// NOTE: Suppressing all warnings is NOT a good practice to follow in general.
// This is only done to remove warnings related to this class' internal logic.
public final class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener,
    ...
    /**
     * Renders the turtles and listens for key presses.
     * This method is run in two separate threads, one for rendering and one for
     * listening.
     *
    ...
    /**
     * This is the main event loop, which is run in a separate thread.
     * It listens for key presses and processes them according to the key bindings.
     * It also regulates the frame rate of the program by sleeping for a calculated
     * amount of time each loop.
     */
    ...
    /**
     * This is the main rendering loop, which is run in a separate thread.
     * It renders the turtles according to the refresh mode and regulates the frame
     * rate of the program
     * by sleeping (waiting) for a calculated amount of time each loop.
     */
    ...
    /**
     * Waits until the specified timestamp (in nanoseconds).
     * 
     * @return true after the time has passed, false if the time has already passed
    ...
    /**
     * <p>
     * Initializes the turtle graphics system.
     * This is called automatically when the first turtle is created.
     * Call this method to manually create the window.
     * </p>
     */
    ...
    /**
     * Exits the turtle graphics system. This will close the underlying window.
     */
    ...
    /**
     * Initializes the color map with the 147 HTML color names and their
     ...
     *
     * The implementation supports looking up color names from the
     * initialized HTML color map (case-insensitive). Hex color codes are not
     * parsed by this method.
     * {@link http://www.w3.org/TR/css3-color/#svg-color|W3C CSS Color Module Level
     * 3} for more information on supported color names.
     * </p>
     * 
    ...
    /**
     * Sets up the buffering system. This is called automatically when the first
     * turtle is created.
     */
    ...
    /**
     * Draws the turtle icon in the upper left corner of the window.
     * This is called automatically when the first turtle is created.
     */
    ...
    /**
     * Creates a fully cloned copy of a turtle.
     *
     ...
     */
    public boolean approxCollidesWith(int x, int y) {
        if (!isVisible)
            return false;
        return contains(x, y);
    }
     ...
     * Sets the direction in such a way that it faces (x,y)
     *
     * @param other another turtle to face
     * @return state change timestamp
     */
    public long face(Turtle other) {
        return setDirection(towards(other.getX(), other.getY()));
    }
     ...
     * @param x x coordinate of target location
     * @param y y coordinate of target location
     * @return angle in degrees where <code>{@literal 0<=angle<360}</code>
     */
    ...
    /**
     * Sets the X position of a turtle.
     *
     * @param x x coordinate
     * @return state change timestamp
     */
    public long setX(double x) {
        return setPosition(x, getY(), direction);
    }

    /**
     * Sets the Y position of a turtle.
     *
     * @param y y coordinate
     * @return state change timestamp
     */
    public long setY(double y) {
        return setPosition(getX(), y, direction);
    }
    ...
    /**
     * Sets the shape of the turtle to a custom size
     * 
     * @param width  width of the turtle shape
     * @param height height of the turtle shape
     * @return state change timestamp
     */
    ...
    /**
     * Adds a key binding to the turtle. Used internally.
     * 
     ...
     * @return true if the method was successfully linked to the key, false if the
     *         method doesn't exist or has the wrong signature.
     */
    @SuppressWarnings("unchecked")
    private boolean addKeyBinding(String methodName, String keyText, boolean append, boolean repeat) {
        keyText = keyText.toLowerCase();
        String className = "";
        // Workaround to find the caller's class name
        try {
     ...
     * Simply pass in the name of a method inside of the class that's calling this
     * method.
     * The method must either take in a Turtle object or a String as a parameter and
     * should return void
     * </p>
     *
     * @param methodName method to be executed when the key is pressed
    ...
    /**
     * Processes the pressed keys and calls the corresponding methods.
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Gets the width of the canvas (i.e., the maximum X value for the turtle before
    ...
    /**
     * Gets the height of the canvas (i.e., the maximum Y value for the turtle
     * before it appears off-screen).
     * 
    ...
    /**
     * Internal method for saving the visible canvas to a .gcode file.
     * 
    ...
    /**
     * Maximizes the window and focuses it.
     */
    ...
    /**
     * Blocks for a given number of seconds.
     * 
    ...
    /**
     * <p>
     * Determines if this turtle approximately collides with another turtle using
     * approximate rectangular hitboxes.
     * </p>
     * 
     * @param other the turtle to check collision with
     * @return true if the turtles collide, false otherwise
     * @see #approxCollidesWith(Turtle, boolean)
     */
    public boolean approxCollidesWith(Turtle other) {
        return approxCollidesWith(other, false);
    }

    /**
     * <p>
     * Determines if this turtle approximately collides with another turtle using
     * approximate hitboxes.
     * - If circular is true, uses two circles to determine collision.
     * - Else, uses two rectangles to determine collision.
     * 
     * Both collision detection methods center the shape at the turtle's location
     * and uses the turtle's shape
     * width and height multiplied by the scale factor to determine the size of the
     * shape.
     * 
     * </p>
     * 
     * @param other    the turtle to check collision with
     * @param circular if true, uses circular collision detection. If false, uses
     *                 rectangular collision detection.
     * @return true if the turtles collide, false otherwise
     */
    public boolean approxCollidesWith(Turtle other, boolean circular) {
        synchronized (turtleLock) {
            if (this != other && this.isVisible && other.isVisible) {
                if (circular) {
                    // Treat both turtles as circular
                    double x1 = _location.x, y1 = _location.y, x2 = other._location.x, y2 = other._location.y;
                    double r1 = Math.max(_shapeWidth, _shapeHeight) * scale / 2;
                    double r2 = Math.max(other._shapeWidth, other._shapeHeight) * scale / 2;
                    return Math.hypot(x1 - x2, y1 - y2) <= r1 + r2;
                } else {
                    // Treat both turtles as rectangles
                    double x1 = _location.x, y1 = _location.y, x2 = other._location.x, y2 = other._location.y;
                    double w1 = _shapeWidth * scale, h1 = _shapeHeight * scale;
                    double w2 = other._shapeWidth * scale, h2 = other._shapeHeight * scale;
